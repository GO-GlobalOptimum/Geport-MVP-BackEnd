...
#             2. 불로은 Url의 내용을 바탕으로 retriever( vector read_db )를 생성한다.
#             3. 이후 생성된 vector read_db를 활용해 각각의 질문들을 llm을 통해서 만들어낸다.
#     '''
#     # /***************************************************************************/#
#     start_time = time.time()
...
async def check_read_db_connection(read_db: Session):
    try:
...
# async def generate_geport(member_id: int, post_ids: List[int], questions: List[str], read_db: Session):
#     # /***************************************************************************/#
#     '''
...
#             3. 선택된 post_id의 post_content를 바탕으로 retriever( vector read_db )를 생성한다.
#             4. 이후 생성된 vector read_db를 활용해 각각의 질문들을 llm을 통해서 만들어낸다.
#     '''
#     # /***************************************************************************/#
...
#     await check_read_db_connection(read_db)
...
async def generate_geport(post_ids: List[int], questions: List[str], read_db: Session):
