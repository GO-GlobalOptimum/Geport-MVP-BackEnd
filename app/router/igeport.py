...
from app.services.igeport_asyncio import create_user_service, read_list_service, read_user_service, generate_igeport


router = APIRouter()

@router.post("/igeport/generate/userInfo", status_code=status.HTTP_201_CREATED)
def create_user(user_data: UserData):
    return create_user_service(user_data)


@router.get("/igeport/read/userInfo/{encrypted_id}")
def read_user(encrypted_id: str):
    return read_user_service(encrypted_id)

@router.get("/igeport/infoList")
def read_list():
    return read_list_service()


@router.get("/igeport/generate/Igeport/GPT/Asyncio/{encrypted_id}")
