...
async def get_recently_viewed_category_post_counts_endpoint(read_db: Session = Depends(get_read_db), current_user: dict = Depends(get_current_user)):
    """
    ...
    """
    result = get_recently_viewed_category_post_counts(read_db, current_user)
    return result
